						...
						<c:forEach items="${page.list }" var="user" varStatus="row">
							...
							<tr>
								<td colspan="7">
									...
									<c:choose>
										<c:when test="${page.hasFirst }"><a href="${pageContext.request.contextPath }/user/user_view.action?currentPag=1">首页</a></c:when>
										<c:otherwise><font style="color: red;">首页</font></c:otherwise>
									</c:choose>
									<c:choose>
										<c:when test="${page.hasPre }"><a href="${pageContext.request.contextPath }/user/user_view.action?currentPage=${page.currentPage - 1 }">上一页</a></c:when>
										<c:otherwise><font style="color: red;">上一页</font></c:otherwise>
									</c:choose>
									<c:choose>
										<c:when test="${page.hasNext }"><a href="${pageContext.request.contextPath }/user/user_view.action?currentPage=${page.currentPage + 1 }">下一页</a></c:when>
										<c:otherwise><font style="color: red;">下一页</font></c:otherwise>
									</c:choose>
										<c:choose>
										<c:when test="${page.hasLast}"><a href="${pageContext.request.contextPath }/user/user_view.action?currentPage=${page.totalPage }">尾页</a></c:when>
